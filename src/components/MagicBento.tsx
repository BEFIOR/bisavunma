...
const MagicBento: React.FC<BentoProps> = ({
  ...
  // Use products data if provided, otherwise use default cardData
  const displayData =
    products && products.length > 0
      ? products.map((product, index) => ({
          color: "#060010",
          title: product.title,
          description: product.description || "Detaylı bilgi için tıklayın",
          label: `Ürün ${index + 1}`,
          slug: product.slug,
          image: product.image,
        }))
          ...
          {displayData.map((card, index) => {
            ...
            if (enableStars) {
              ...
              return card.slug ? (
                <Link
                  key={index}
                  href={`/urunler/radar-sistemleri/${card.slug}`}
                >
                  {cardElement}
                </Link>
              ) : (
            ...
            return card.slug ? (
              <Link key={index} href={`/urunler/radar-sistemleri/${card.slug}`}>
                {cardElement}
              </Link>
            ) : (
              cardElement
